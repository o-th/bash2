/*
 if (angles[Current][Yaw] == angles[Last][Yaw])
            {
                if (angles[Current][Pitch] == angles[Last][Pitch])
                {
                    checkMovement(client, vel[mv], mv, "+strafe");
                    return;
                }
               
                checkMovement(client, vel[mv], mv, "lookstrafe (blocked yaw)");
                return;
            }
           
            if (angles[Current][Pitch] == angles[Last][Pitch])
            {
                checkMovement(client, vel[mv], mv, "lookstrafe (blocked pitch)");
                return;
            }
*/

// In CheckForIllegalTurning func, check if player has sidemove, if not they then they might be using a controller
...
public Plugin myinfo = 
{
	...
	version = "2.0",
	url = "http://steamcommunity.com/id/mev"
};
...
float g_fAngleDifference[MAXPLAYERS + 1];
float g_fLastAngleDifference[MAXPLAYERS + 1];
...
public Action Event_PlayerJump(Event event, const char[] name, bool dontBroadcast)
{
	...
	if(++g_iJump[client] == 6)
	{
		...
		float yawPct = (float(g_iYawTickCount[client]) / float(g_strafeTick[client])) * 100.0;
		float timingPct = (float(g_iTimingTickCount[client]) / float(g_strafeTick[client])) * 100.0;
		if(gainPct > 85.0 && yawPct < 60.0)
		{
			PrintToAdmins("%N has %.2f%% gains (Yawing %.1f%, Timing: %.1f%)", client, gainPct, yawPct, timingPct);
			AnticheatLog("%L has %.2f% gains (Yawing %.1f%, Timing: %.1f%)", client, gainPct, yawPct, timingPct);
		...
		else if(g_iKeyboardSampleTime[client] == 0 && gainPct >= 70.0)
		{
			PrintToAdmins("%N has %.2f%% (Yawing %.1f%, Timing: %.1f%) gains while in_usekeyboardsampletime is set to 0", client, gainPct, yawPct, timingPct);
			AnticheatLog("%L has %.2f% (Yawing %.1f%, Timing: %.1f%) gains while in_usekeyboardsampletime is set to 0", client, gainPct, yawPct, timingPct);
		}
...
public Action OnPlayerRunCmd(int client, int &buttons, int &impulse, float vel[3], float angles[3], int &weapon, int &subtype, int &cmdnum, int &tickcount, int &seed, int mouse[2])
{
	if(!IsFakeClient(client) && IsPlayerAlive(client))
	{
		...
		if(bCheck)
		{
			...
			CheckForOpti(client, vel, angles);
		}
...
void CheckForIllegalTurning(int client, float vel[3])
{
	...
	if(g_iCmdNum[client] % 100 == 0)
	{
		...
		{
			PrintToAdmins("%N is turning with illegal yaw values (%f, %f, %d, %d)", client, g_mYaw[client], g_Sensitivity[client], g_iIllegalYawCount[client], g_mYawChangedCount[client]);
			AnticheatLog("%L is turning with illegal yaw values (%f, %f, %d, %d)", client, g_mYaw[client], g_Sensitivity[client], g_iIllegalYawCount[client], g_mYawChangedCount[client]);
		}
	...
	if(FloatAbs(g_fAngleDifference[client]) < 0.01)
	{
		return;
	}
	...
	if(FloatAbs(g_fAngleDifference[client]) > 20.0 || FloatAbs(g_Sensitivity[client] * g_mYaw[client]) > 0.8)
	{
		return;
	}
	...
	else if(g_mCustomAccel[client] == 0)
	{
		float fMove = g_fAngleDifference[client] / (g_mYaw[client] * g_Sensitivity[client]);
		//int iFOV = GetEntProp(client, Prop_Send, "m_iFOV");
		//if(iFOV < 90) fMove *= (float(90) / float(iFOV));// * g_ZoomSensitivity[client];
		if(g_Engine == Engine_CSS) fMove *= 2;
		if(g_mFilter[client] == true) fMove *= 2;
		 // Takes into account players that are zooming in
		float fRounded = float(RoundFloat(fMove));
		
		if(FloatAbs(fRounded - fMove) > 0.01)
		{
			g_iIllegalYawCount[client]++;
		}
	}	
...
void CheckForOpti(int client, float vel[3], float angles[3])
{
	...
	static int g_iLastStrength[MAXPLAYERS + 1];
	static int g_iSameStrengthCount[MAXPLAYERS + 1];
	
	float optimal_delta_yaw = GetOptimalDeltaYaw(client, vel, angles);
	float strength = FloatAbs( g_fAngleDifference[client] / optimal_delta_yaw );
	
	//PrintToAdmins( "Strength value: %.2f", strength );
	
	int roundedStrength = RoundFloat( strength );
	if( FloatAbs( float(g_iLastStrength[client] - roundedStrength) ) <= 1.0 )
	{
		g_iSameStrengthCount[client]++;
		
		// now check if they have too many suspiciously consistent strafe strength
	}
	else
	{
		g_iSameStrengthCount[client] = 0;
		g_iLastStrength[client] = roundedStrength
	}
}

float GetOptimalDeltaYaw(int client, float vel[3], float angles[3])
{
	float frwd[3], right[3];
	GetAngleVectors( angles, frwd, right, NULL_VECTOR );

	frwd[2] = 0.0;
	right[2] = 0.0;
	NormalizeVector( frwd, frwd );
	NormalizeVector( right, right );
	
	float wishvel[3];
	for( int i = 0; i < 2; i++ )
	{
		wishvel[i] = frwd[i]*vel[0] + right[i]*vel[1];
	}
	
	float wishspeed = GetVectorLength( wishvel );
	if( wishspeed > FindConVar( "sv_maxspeed" ).FloatValue )
	{
		wishspeed = FindConVar( "sv_maxspeed" ).FloatValue;
	}
	
	float optimal_delta_yaw = angles[1] - MaxAccelTheta( client, wishspeed, vel, angles );
	if( optimal_delta_yaw >= 180.0 )
	{
		optimal_delta_yaw -= 360.0;
	}
	else if( optimal_delta_yaw < -180.0 )
	{
		optimal_delta_yaw += 360.0;
	}
	
	return FloatAbs( optimal_delta_yaw );
}

stock float MaxAccelTheta( int client, float wishspeed, float vel[3], float angles[3] )
{
	float accel = FindConVar( "sv_airaccelerate" ).FloatValue;
	float accelspeed = accel * wishspeed * GetTickInterval() * GetEntPropFloat( client, Prop_Data, "m_flLaggedMovementValue" );
	if( accelspeed <= 0.0 )
	{
		return 180.0;
	}
	...
	if( absvel[0] == 0.0 && absvel[1] == 0.0 )
	{
		return 0.0;
	}
	
	float tmp = 30.0 - accelspeed;
	if( tmp <= 0.0 )
	{
		return 90.0;
	}
	
	float speed = SquareRoot( absvel[0]*absvel[0] + absvel[1]*absvel[1] );
	
	if( tmp < speed )
	{
		return RadToDeg( ArcCosine( tmp / speed ) );
	}
...
{	
	...
	if(FloatAbs(g_fAngleDifference[client] - g_fLastAngleDifference[client]) > 13 && // Player turned more than 13 degrees in 1 tick
	g_fAngleDifference[client] != 0.0 && 
	...
	g_iIllegalTurn_IsTiming[client][g_iIllegalTurn_CurrentFrame[client]] = (Shavit_GetTimerStatus(client) == Timer_Running);
	...
	if(g_iIllegalTurn_CurrentFrame[client] == 0)
	{
		...
		{
			char sStyle[32];
			GetClientStyleName(client, sStyle, sizeof(sStyle));
			PrintToAdmins("%N angle snap hack, Pct: %.2f%, Timing: %.1f%, Style: %s", client, illegalPct * 100.0, timingPct * 100.0, sStyle);
			AnticheatLog("%L angle snap hack, Pct: %.2f%, Timing: %.1f%, Style: %s", client, illegalPct * 100.0, timingPct * 100.0, sStyle);
		}
	}
...
{
	if(g_fAngleDifference[client] == 0.0 && g_bIsTurning[client] == true)
	{
		g_iLastStopTurnTick[client] = g_iCmdNum[client];
		g_bIsTurning[client]        = false;
	}
	else if(g_fAngleDifference[client] > 0)
	{
...
stock void RecordStartStrafe(int client, int button, int turnDir, const char[] caller)
{
	...
	g_iStartStrafe_Stats[client][currFrame][StrafeData_IsTiming]      = (Shavit_GetTimerStatus(client) == Timer_Running);
	...
	if(g_iStartStrafe_CurrentFrame[client] == 0)
	{
		...
		if(sd < 0.8)
		{
			...
			PrintToAdmins("%N start strafe, avg: %.2f, dev: %.2f, Timing: %.1f%, style: %s", client, mean, sd, timingPct * 100, sStyle);
			AnticheatLog("%L start strafe, avg: %.2f, dev: %.2f, Timing: %.1f%, style: %s", client, mean, sd, timingPct * 100, sStyle);
...
stock void RecordEndStrafe(int client, int button, int turnDir, const char[] caller)
{
	...
	g_iEndStrafe_Stats[client][currFrame][StrafeData_IsTiming]      = (Shavit_GetTimerStatus(client) == Timer_Running);
	...
	if(g_iEndStrafe_CurrentFrame[client] == 0)
	{
		...
		if(sd < 0.8)
		{
			...
			GetClientStyleName(client, sStyle, sizeof(sStyle));
			float timingPct = float(timingCount) / float(MAX_FRAMES);
			PrintToAdmins("%N end strafe, avg: %.2f, dev: %.2f, Timing: %.1f%, style: %s", client, mean, sd, timingPct * 100, sStyle);
			AnticheatLog("%L end strafe, avg: %.2f, dev: %.2f, Timing: %.1f%, style: %s", client, mean, sd, timingPct * 100, sStyle);
...
stock void RecordKeySwitch(int client, int button, int oppositeButton, int btype, const char[] caller)
{
	...
	g_iKeySwitch_Stats[client][currFrame][KeySwitchData_IsTiming][btype]    = (Shavit_GetTimerStatus(client) == Timer_Running);
...
void CheckForIllegalMovement(int client, float vel[3], int buttons)
{
	...
	*/
	...
	if(RoundToFloor(vel[0] * 100.0) % 625 != 0 || RoundToFloor( vel[1] * 100.0 ) % 625 != 0)
	{
		...
		if(FloatAbs(g_fAngleDifference[client]) > 0)
		{
...
{
	g_fAngleDifference[client] = angles[1] - g_fLastAngles[client][1];
		
	if (g_fAngleDifference[client] > 180)
		g_fAngleDifference[client] -= 360;
	else if(g_fAngleDifference[client] < -180)
		g_fAngleDifference[client] += 360;
}
...
stock void GetClientStyleName(int client, char[] buffer, int maxlen)
{
	int style = Shavit_GetBhopStyle(client);
	Shavit_GetStyleStrings(style, sStyleName, buffer, maxlen);
}
